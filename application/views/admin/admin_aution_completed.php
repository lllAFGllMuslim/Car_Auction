...
if(!empty($cars)){
  ...
  <tr class="sert54">
    <td class="head15700 ader_br hj15">
      <img src="<?php if(!empty($profile_data['profle_photo_id'])){ echo get_image_path_by_id($profile_data['profle_photo_id']); }else{ ?><?= base_url(); ?>assets/img/us3.jpg<?php } ?>" width="50px" alt="" />
    </td>
    ...
    <td class="head15700 ader_br">
      <a href="<?php echo base_url(); ?>car/<?php echo $car->car_slug; ?>">
        <img src="<?php if (!empty($image)) { echo get_image_path_by_id($image); } ?>" width="80px" alt="" />
      </a>
    </td>
    ...
    <td class="head15700 ader_br text-center">
      <a href="#view_wrap<?php echo $car->id; ?>" data-bs-toggle="modal" class="view_new" title="Change winner">
  ...
  <?php 
    // Reset user_data for next iteration
    unset($user_data);
  }
...
if(!empty($cars)){
  ...
  foreach ($cars as $car){
    ...
    // Get winner data for the modal header (current winner)
    $winner_data = ['first_name' => 'No Winner', 'last_name' => '', 'phone_number' => ''];
    $winner_bid = get_bid_by_primary_id($car->winner_id);
    if (!empty($winner_bid) && isset($winner_bid->user_id)) {
        $fetched = get_profile($winner_bid->user_id);
        if (!empty($fetched)) {
            $winner_data = $fetched;
        }
    }
      ...
      <div class="row">
        ...
        <?php if(!empty($cardata)){ 
          ...
          foreach ($cardata as $value) {
            ...
            // FIX: Safely get car data and check for null before accessing as array
            $car_data = get_car_by_id($value->car_id);
            $is_winner = (!empty($car_data) && isset($car_data["winner_id"]) && $car_data["winner_id"] == $value->id);

            // FIX: Get each bid's own bidder profile instead of always showing winner
            $bid_profile = get_profile($value->user_id);
            if (empty($bid_profile)) {
                $bid_profile = ['first_name' => 'Unknown', 'last_name' => '', 'phone_number' => ''];
            }
            $bid_display_name = $bid_profile["first_name"] . ' ' . $bid_profile["last_name"];
                ...
                <div class="leading_wrap15">
                  <div class="leading_bh">
                    <div class="call_bid958">
                      <div class="namj55"><?php echo $bid_display_name; ?></div>
                      <i class="fa fa-user"></i> LEADING BID
                      <span class="ad52 ml_a55"><?php echo $value->unique_id; ?></span>
                    </div>
                  ...
                  <div class="selec_wrap">
                    <input <?php if($is_winner){ echo "checked"; } ?> class="winner" name="winner" id="<?php echo $value->car_id; ?>" data-winid="<?php echo $value->id; ?>" type="radio" value="<?php echo $value->car_id; ?>">
                    Select winner
                  </div>
                </div>
              </div>
                ...
                <div class="leading_wrap16">
                  <div class="leading_bh">
                    ...
                    <div class="call_bid959">
                      ...
                      <span class="ad52"><?php echo $value->unique_id; ?></span>
                    </div>
